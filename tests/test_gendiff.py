...
def test_generate_diff():
    ...
    with open("tests/fixtures/test_answer_stylish.txt", "r") as test_answer:
        assert generate_diff(file1_json, file2_json, format = 'stylish') == test_answer.read()
        assert generate_diff(file1_yaml, file2_yaml, format = 'stylish') == test_answer.read()
    with open("tests/fixtures/test_answer_plain.txt", "r") as test_answer:
        assert generate_diff(file1_json, file2_json, format = 'plain') == test_answer.read()
        assert generate_diff(file1_yaml, file2_yaml, format = 'plain') == test_answer.read()
    with open("tests/fixtures/test_answer_json.txt", "r") as test_answer:
        assert generate_diff(file1_json, file2_json, format = 'json') == test_answer.read()
        assert generate_diff(file1_yaml, file2_yaml, format = 'json') == test_answer.read()
